...
function mountEmailForm(elementId, apiEndpoint) {
  const root = (0, import_client.createRoot)(document.getElementById(elementId));
  root.render(/* @__PURE__ */ import_react.default.createElement(EmailForm, { apiEndpoint }));
}
var emailRE = /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9-]+(?:.[a-zA-Z0-9-]+)*$/;
var EmailForm = (props) => {
  const [emailText, setEmailText] = (0, import_react.useState)("");
  const [apiStatus, setApiStatus] = (0, import_react.useState)("idle");
  const refState = (0, import_react.useRef)(apiStatus);
  refState.current = apiStatus;
  function submitForm(event) {
    event.preventDefault();
    if (refState.current === "loading") {
      return;
    }
    ...
    fetch(props.apiEndpoint, {
      ...
      headers: {
        ...
        authorization: "Basic " + btoa("bongio:Iwi113dr!")
      },
  ...
  const isEmailValid = (0, import_react.useMemo)(() => emailRE.test(emailText), [emailText]);
  return /* @__PURE__ */ import_react.default.createElement("form", { action: "#", method: "post", className: "space-y-3 md:space-y-6" }, /* @__PURE__ */ import_react.default.createElement("div", null, /* @__PURE__ */ import_react.default.createElement(
    "input",
    {
      type: "email",
      name: "email",
      ...
      value: emailText,
      className: `w-full px-4 py-2 border ${isEmailValid ? "border-green-700" : "border-gray-300"} rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500`
    }
  )), /* @__PURE__ */ import_react.default.createElement(
    "button",
    {
      type: "submit",
      disabled: !isEmailValid || apiStatus === "loading" || apiStatus === "success",
      className: `relative w-full px-4 py-2 ${apiStatus === "error" ? "bg-red-600 hover:bg-red-700" : apiStatus === "success" ? "bg-green-600 hover:bg-green-700" : apiStatus === "loading" || apiStatus === "success" ? "disabled:bg-primary-500 bg-primary-500" : "bg-primary-600 hover:bg-primary-700 "} text-white rounded-lg transition-colors duration-200`,
      onClick: submitForm
    },
    apiStatus === "loading" && /* @__PURE__ */ import_react.default.createElement("div", { className: "absolute top-0 bottom-0 left-8 flex items-center" }, /* @__PURE__ */ import_react.default.createElement(
      "svg",
      {
      ...
      /* @__PURE__ */ import_react.default.createElement(
        "path",
        {
          ...
          fill: "currentColor",
          d: "M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A8 8 0 0020 12H16c0 3.042-1.5 5.85-4 7.291l2-3.464h-6z"
        }
      )
    )),
    apiStatus === "success" ? "Thanks for registering!" : apiStatus === "error" ? "Sorry, something went wrong" : "Register for early access"
  ));
};
export {
  mountEmailForm
